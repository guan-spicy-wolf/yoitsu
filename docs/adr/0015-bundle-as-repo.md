...
├── contexts/       # context provider（palimpsest runtime 加载）
├── prompts/        # prompt 文件
...
其中 `roles/` `tools/` `contexts/` 为 palimpsest runtime 强依赖的最小必需集合，Phase 1 bundle **必须**提供；`prompts/` 为默认约定目录（绝大多数 bundle 会使用），但不是绝对必需（role 可内嵌 prompt 或引用外部 URI）；`scripts/` `lib/` `examples/` 为 bundle 自主决定的扩展空间。
