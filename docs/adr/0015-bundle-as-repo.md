...
bundles:
  ...
  webdev:
    source:
      url: "git+https://github.com/.../webdev-bundle.git"
      selector: main
```
...
- `git+https://github.com/guan-spicy-wolf/factorio-bundle.git@a1b2c3d#prompts/worker.md` — 指向特定 commit 下的 worker prompt 文件
- `git+https://github.com/guan-spicy-wolf/factorio-bundle.git@a1b2c3d#scripts/place_entity.lua` — 指向特定 commit 下的脚本
...
- **`palimpsest/docs/design.md`**：原文将 "evo repo 作为自由表面" 作为 runtime 的中心概念，本 ADR 之后它应改写为 "runtime 消费由 trenni 准备的双 workspace（BundleSource.workspace 加载代码 + TargetSource.workspace 执行任务）；evo 概念在 runtime 视角消失"。
...
Publication 的 push 策略在 git_workspace capability 的 finalize 中实现。

**同步 push，返回 FinalizeResult(events, success)**：
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            events.append(EventData(type="artifact.published", data={
                "ref": f"{ctx.target_source.repo_uri}@{sha.strip()}",
...
**Hallucination Gate 语义**：

| 检出结果 | capability 返回 | job 终态 |
|---|---|---|
| 有变更，push 成功 | `success=True` | `job.completed` |
| 有变更，push 失败 | `success=False` | `job.failed` |
| 无变更（hallucination） | `success=False` | `job.failed` |

**关键**：Worker role 应有产出，无产出 = hallucination = `success=False` = job.failed。Planner/Evaluator 不修改文件，应配置 `strategy="skip"` 或使用不同的 capability。

**Artifact URI 指向远端**：

- `artifact.published.ref` 必须指向远端仓库 URI（如 `git+https://github.com/org/repo@sha`）
- **不能**指向 ephemeral workspace 路径（如 `git+file:///tmp/workspace@sha`）
- workspace job 结束后被回收，URI 失效
...
3. Per-bundle 并发上限的配置位置（bundle.yaml 自声明 vs trenni config 覆盖）
4. Bundle repo 的 secret 注入（环境变量键名约定，如 `YOITSU_BUNDLE_GIT_TOKEN`）
5. `evolve` 分支约定的强制程度（ADR 推荐但不强制，细节交给 bundle convention 文档）
6. 新 factorio-bundle 仓库的物理落地路径（交给运维约定）
