...
This single primitive expresses: sequential chains, parallel batches, conditional branching, nested forks, and iteration.

**The spawn tool emits a `job.spawn.request` event; it does not execute child jobs.** The Supervisor owns fork-join lifecycle entirely. This keeps the Runtime as a single-job engine with no orchestration responsibility — an earlier design had spawn executing children inline, which coupled the Runtime to Supervisor concerns and gave the Agent false promises about child results.

**Job IDs are assigned by the Supervisor before launch, never generated by the Runtime.** Spawn requests lock child tasks to the initiating `evo_sha` to prevent version drift during parallel execution. This makes all job lifecycle events and sub-branches strictly traceable to a globally managed identity.
