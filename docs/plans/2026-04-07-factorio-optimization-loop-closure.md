...
- [x] 5.3 验证文件：`ls evo/factorio/scripts/` 应包含新脚本；内容看起来像 radius scan 实现（功能完美与否不验证）。
  ...
  - 但文件未出现在 `evo/factorio/scripts/`，也未出现在 live mod scripts 目录 `/home/holo/factorio/mods/factorio-agent_0.1.0/scripts/`
- [ ] 5.4 第二轮：再触发同一任务。worker preparation 应触发 reload，使新脚本生效。**阻塞：** 需先解决 5.3 的产物缺失问题。
